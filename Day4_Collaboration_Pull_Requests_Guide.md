...
- GitHub account with SSH configured (see Day 3).
- Trainer’s sample repo URL and (for Task 1) your fork’s SSH clone URL.

**Clone your fork** (replace URL and folder name with yours):
```bash
git clone git@github.com:jjnavsofs0/sample-repo.git
cd sample-repo
```

**Windows:** Same commands in Git Bash; or `cd` into the cloned folder from Explorer after cloning.

---

## Lab Task Checklist

Condensed commands. For UI steps (fork, open PR, review, merge), see the [Lab Guide](#lab-guide).

### Task 1: Fork & Add Upstream
Fork the trainer’s repo in the browser, then:
```bash
git clone git@github.com:jjnavsofs0/sample-repo.git
cd sample-repo
git remote -v
...
### Task 2: Feature Branch & Commit
```bash
git checkout -b feature/add-jjnavsofs0-function
# Edit calculator.py and tests — see Lab Guide
git add -A
git commit -m "feat: add power() function"
```
...
```bash
git push -u origin feature/add-jjnavsofs0-function
# Open PR in browser: base = upstream main, compare = your branch
```

### Task 4: Review a Peer’s PR
Complete in GitHub/GitLab UI (Files changed, inline comments, Submit review).

### Task 5: Address Feedback & Merge
```bash
git add -A
git commit -m "fix: address review feedback"
git push
# After merge on remote:
git checkout main
git pull origin main
git branch -d feature/add-jjnavsofs0-function
```

**Deliverables:** Fork · PR opened · Review activity · Merged contribution
